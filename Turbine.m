classdef Turbine < handle
    ...
    properties
        ...
        annual_average_power                                                                           %机组的年平均发电功率
        past_comprehensive_fatigue_coefficient                                                          %优化前的综合疲劳系数
    ...
    properties (Constant)
        points_turbine_grid=25;%风机(转子)网格点
    end
    ...
    methods
        %% 获取当前风机的转子直径和轮毂高度
        function obj=Turbine(sqz_12,turbine_diameter_vector,turbine_hub_height_vector,rated_power_vector,life_total_vector,repair_c_vector,fatigue,count_tn)
            s_qz_12=sqz_12; 
            qz_3_count=table2array(readtable('qingzhou3_layout_1.xlsx'));
        ...
        obj.past_comprehensive_fatigue_coefficient=fatigue(count_tn);
        obj.annual_average_power=obj.rated_power*0.7;
        %% Cp-CT表(不同型号机组对应不同表)
...
%%%%%%%%更新当前风机关联点处的风速和湍流强度%%%%%%%%%%%%
        ...
        function comprehensive_fatigue_coefficient=get.comprehensive_fatigue_coefficient(obj)
            ...
            % 剩余寿命比
            comprehensive_fatigue_coefficient=optimized_c+accumulated_c;
        end  
        ...
        %% 不考虑寿命的发电量
        function single_turbine_generation=get.single_turbine_generation(obj)
            %%尽可能最大化1)正常所有风机的发电量+2)由各机组寿命优化折算得到的发电量的总和
